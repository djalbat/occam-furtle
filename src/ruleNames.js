"use strict";

export const ALL_RULE_NAME = "all";
export const SOME_RULE_NAME = "some";
export const TERM_RULE_NAME = "term";
export const TYPE_RULE_NAME = "type";
export const TERMS_RULE_NAME = "terms";
export const EVERY_RULE_NAME = "every";
export const LABEL_RULE_NAME = "label";
export const ERROR_RULE_NAME = "error";
export const VALUE_RULE_NAME = "value";
export const EXISTS_RULE_NAME = "exists";
export const VALUES_RULE_NAME = "values";
export const REDUCE_RULE_NAME = "reduce";
export const TERNARY_RULE_NAME = "ternary";
export const BINDING_RULE_NAME = "binding";
export const VARIABLE_RULE_NAME = "variable";
export const NONSENSE_RULE_NAME = "nonsense";
export const DOCUMENT_RULE_NAME = "document";
export const BINDINGS_RULE_NAME = "bindings";
export const CONTAINS_RULE_NAME = "contains";
export const STATEMENT_RULE_NAME = "statement";
export const PROCEDURE_RULE_NAME = "procedure";
export const LENGTH_OF_RULE_NAME = "lengthOf";
export const PRIMITIVE_RULE_NAME = "primitive";
export const REFERENCE_RULE_NAME = "reference";
export const PARAMETER_RULE_NAME = "parameter";
export const ENDS_WITH_RULE_NAME = "endsWith";
export const REFERENCES_RULE_NAME = "references";
export const NODE_QUERY_RULE_NAME = "nodeQuery";
export const EXPRESSION_RULE_NAME = "expression";
export const PARAMETERS_RULE_NAME = "parameters";
export const TO_INTEGER_RULE_NAME = "toInteger";
export const STARTS_WITH_RULE_NAME = "startsWith";
export const TRY_INTEGER_RULE_NAME = "tryInteger";
export const NODES_QUERY_RULE_NAME = "nodesQuery";
export const RETURN_BLOCK_RULE_NAME = "returnBlock";
export const LOGICAL_TERM_RULE_NAME = "logicalTerm";
export const NEGATED_TERM_RULE_NAME = "negatedTerm";
export const NAMED_BINDING_RULE_NAME = "namedBinding";
export const NAMED_BINDINGS_RULE_NAME = "namedBindings";
export const BRACKETED_TERM_RULE_NAME = "bracketedTerm";
export const PROCEDURE_CALL_RULE_NAME = "procedureCall";
export const IMPORT_BINDING_RULE_NAME = "importBinding";
export const COMPARISON_TERM_RULE_NAME = "comparisonTerm";
export const LIST_ASSIGNMENT_RULE_NAME = "listAssignment";
export const IMPORT_BINDINGS_RULE_NAME = "importBindings";
export const RETURN_STATEMENT_RULE_NAME = "returnStatement";
export const IMPORT_STATEMENT_RULE_NAME = "importStatement";
export const OBJECT_ASSIGNMENT_RULE_NAME = "objectAssignment";
export const VARIABLE_ASSIGNMENT_RULE_NAME = "variableAssignment";
export const ANONYMOUS_PROCEDURE_RULE_NAME = "anonymousProcedure";
export const VARIABLE_ASSIGNMENTS_RULE_NAME = "variableAssignments";
export const REFERENCE_LIST_LITERAL_RULE_NAME = "referencesListLiteral";
